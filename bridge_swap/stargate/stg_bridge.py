...
def mass_stg_transfer(config_data: ConfigSchema):
    ...
    token_bridge = StgBridge(config=config_data)
    wallet_number = 1
...
class StgBridge(BridgeBase):
    ...
    def transfer(self, private_key, wallet_number=None):
        ...
        wallet_token_balance_decimals = wallet_token_balance_wei / 10 ** self.get_token_decimals(self.src_token_contract)
        ...
        txn = self.build_stg_bridge_txn(amount_out=token_amount_out,
                                        src_wallet_address=source_wallet_address,
                                        dst_wallet_address=dst_wallet_address,
                                        dst_cain_id=self.target_chain.chain_id,
